...
class Task:
    ...
    def build(self):
        ...
        for command in self.commands:
            ...
            combinedOutput = (out + err).decode("utf-8", errors="replace").strip()
...
if len(tasks):
    threads = []
    # The build always passes -parallel, but an all-logical-core fan-out can
    # saturate the desktop independently of Ninja's -j limit.
    threadCount = min(multiprocessing.cpu_count(), 4) if args.parallel else 1

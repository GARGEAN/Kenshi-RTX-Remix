...
namespace fork_hooks {
  ...
  namespace {
    ...
    void fhSyncAtmosphereDistantLights(RtxContext& ctx, const AtmosphereArgs& args) {
      ...
      if (!numosActive && !kenshiSunActive) {
      ...
      {
        // Direction: from the atmosphere args in Numos, or rebuilt from the
        // game-driven options on the Kenshi path using the same convention
        // getAtmosphereArgs() uses, so both paths agree.
        Vector3 sunDirYUp(args.sunDirection.x, args.sunDirection.y, args.sunDirection.z);
        if (!numosActive) {
          constexpr float kDegToRad = kFhPi / 180.0f;
          const float azimuthRad = RtxOptions::sunRotation() * kDegToRad;
          const float elevationRad = RtxOptions::sunElevation() * kDegToRad;
          sunDirYUp = Vector3(std::cos(elevationRad) * std::sin(azimuthRad),
        ...
        if (!numosActive) {
          // Kenshi path. sunRadianceTint already carries the game's sun hue at
          // preserved luminance TIMES its intensity, normalised so full daylight
          // is ~1.0 and night is exactly 0 - so the day/night fade is inherited
          // rather than reimplemented. The horizon test below is belt-and-braces
          // for the case where the game reports a colour with the sun already
          // set.
          if (sunDirYUp.y > 0.0f) {
            const Vector3 tint = RtxOptions::sunRadianceTint();
            const float base = std::max(RtxOptions::kenshiSunRadiance(), 0.0f) * radScale;
            radiance = Vector3(std::max(tint.x, 0.0f) * base,
                               std::max(tint.y, 0.0f) * base,
                               std::max(tint.z, 0.0f) * base);
          }
        } else if (sunDirYUp.y > 0.0f) {
          ...
          radiance = Vector3(radiance.x * std::max(sunTint.x, 0.0f),
                             ...
                             radiance.z * std::max(sunTint.z, 0.0f));
        }
        ...
        const float physicalHalfAngle = numosActive
          ? args.sunAngularRadius
          : (RtxOptions::sunSize() * (kFhPi / 180.0f) * 0.5f);
  ...
  void syncAtmosphereDistantLights(RtxContext& ctx, const AtmosphereArgs& args) {
    fhSyncAtmosphereDistantLights(ctx, args);
  }
  ...
  void updateAtmosphereConstants(RtxContext& ctx, RaytraceArgs& constants) {
    ...
    // would set constants.skyMode=PhysicalAtmosphere while leaving m_atmosphere
    // null and atmosphereArgs unwritten - the shader would take the atmosphere
    // branch with no LUTs behind it and the sky would go black a different way.
    if (currentSkyMode == SkyMode::PhysicalAtmosphere) {
      if (!ctx.m_atmosphere) {
        ctx.m_atmosphere = std::make_unique<RtxAtmosphere>(ctx.m_device.ptr());
      }
  ...
  namespace {
    ...
    RemixGui::ComboWithKey<SkyMode> skyModeCombo {
      ...
      RemixGui::ComboWithKey<SkyMode>::ComboEntries { {
          ...
          {SkyMode::PhysicalAtmosphere, "Numos"}
      } }
    };

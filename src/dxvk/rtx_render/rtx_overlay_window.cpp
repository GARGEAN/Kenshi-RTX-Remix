...
void GameOverlay::setInputCapture(bool enable) {
  ...
  if (enable) {
    ...
    if (kenshi_telemetry::enabled() && s_captureLogCount < 12) {
      ++s_captureLogCount;
      KENSHI_DIAGNOSTIC_INFO(str::format("[RemixOverlay] raw input captured for menu (saved ",
  ...
  if (kenshi_telemetry::enabled() && s_releaseLogCount < 12) {
    ++s_releaseLogCount;
    KENSHI_DIAGNOSTIC_INFO("[RemixOverlay] raw input released back to the game");
  }
}

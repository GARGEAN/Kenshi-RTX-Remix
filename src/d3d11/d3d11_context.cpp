...
#include "d3d11_terrain_restore_telemetry.inl"
...
namespace dxvk {
  ...
  namespace {
    ...
    bool isValidD3D11Viewport(const D3D11_VIEWPORT& vp) {
      if (!isFiniteViewportValue(vp.TopLeftX)
       || !isFiniteViewportValue(vp.TopLeftY)
       || !isFiniteViewportValue(vp.Width)
       || !isFiniteViewportValue(vp.Height)
       || !isFiniteViewportValue(vp.MinDepth)
       || !isFiniteViewportValue(vp.MaxDepth))
        return false;
      ...
      return isFiniteViewportValue(right)
          && isFiniteViewportValue(bottom)
          && vp.TopLeftX >= D3D11_VIEWPORT_BOUNDS_MIN
          && vp.TopLeftY >= D3D11_VIEWPORT_BOUNDS_MIN
          && right <= D3D11_VIEWPORT_BOUNDS_MAX
          && bottom <= D3D11_VIEWPORT_BOUNDS_MAX;
    }

    D3D11_VIEWPORT sanitizeD3D11Viewport(const D3D11_VIEWPORT& vp) {
      if (!isValidD3D11Viewport(vp))
        return D3D11_VIEWPORT { };

      D3D11_VIEWPORT result = vp;
      result.MinDepth = clampViewportDepth(result.MinDepth);
      result.MaxDepth = clampViewportDepth(result.MaxDepth);
      return result;
    }
  ...
  : D3D11DeviceChild<ID3D11DeviceContext4>(pParent),
    ...
    terrain_restore::VerifyFn verifyTerrain = nullptr;
    {
      verifyTerrain = [](const void* context, terrain_restore::VerificationBoundary boundary) {
        auto* frontend = const_cast<D3D11DeviceContext*>(static_cast<const D3D11DeviceContext*>(context));
        auto before = std::make_shared<std::vector<uint8_t>>();
        const uint64_t identity = terrain_restore::validationIdentity(context);
        frontend->EmitCs([before](DxvkContext* ctx) {
          *before = terrain_restore::snapshotNativeState(ctx);
        });
        // Independent oracle: the existing full restore establishes the CURRENT
        // frontend state at this exact command-stream position. Compare bindings
        // before/after it, never just the pending boolean. Verification is costly
        // and must not be used for performance acceptance.
        frontend->RestoreState();
        frontend->EmitCs([before, context, identity, boundary](DxvkContext* ctx) {
          const auto after = terrain_restore::snapshotNativeState(ctx);
          const bool equal = *before == after;
          if (terrain_restore::validationResult(context, identity, equal)) {
            const auto first = std::mismatch(before->begin(), before->end(), after.begin(), after.end());
            Logger::err(str::format("[TerrainRestore V691] native-state verification mismatch context=",
              context, " byteOffset=", std::distance(before->begin(), first.first),
              " beforeByte=", first.first != before->end() ? uint32_t(*first.first) : 256u,
              " afterByte=", first.second != after.end() ? uint32_t(*first.second) : 256u,
              " boundary=", terrain_restore::reasonName(boundary.reason),
              " lastCompletedFrame=", boundary.lastCompletedFrame,
              " scope=", boundary.scope,
              " pendingBeforeDrain=", boundary.pendingBeforeDrain,
              " deferralEnabled=", boundary.deferralEnabled,
              " beforeCS=", terrain_restore::snapshotComputeShader(*before),
              " afterCS=", terrain_restore::snapshotComputeShader(after)));
          }
        });
      };
    }
    terrain_restore::initialize(this, [](const void* context) {
      const_cast<D3D11DeviceContext*>(static_cast<const D3D11DeviceContext*>(context))->RestoreState();
    }, mode.defer, verifyTerrain, mode.verify);
  }
  
  
  D3D11DeviceContext::~D3D11DeviceContext() {
    terrain_restore::forget(this);
  ...
  void STDMETHODCALLTYPE D3D11DeviceContext::ClearState() {
    ...
    // A submitted draw precedes this call in command-stream order. Give the
    // RTX bridge the still-valid output target before this method clears the
    // D3D11 state that identifies it.
    m_rtx.OnClearState();
  ...
  void STDMETHODCALLTYPE D3D11DeviceContext::ClearRenderTargetView(
    ...
    D3D11DeviceLock lock = LockContext();
    terrain_restore::demand(this, terrain_restore::Reason::Other);
  ...
  void STDMETHODCALLTYPE D3D11DeviceContext::DrawAuto() {
    ...
    if (!m_rtx.OnDrawAuto())
      return;
    terrain_restore::demand(this, terrain_restore::Reason::NativeDraw);
  ...
  void STDMETHODCALLTYPE D3D11DeviceContext::Dispatch(
    ...
    D3D11DeviceLock lock = LockContext();
    terrain_restore::demand(this, terrain_restore::Reason::Dispatch);
    if (IsPredicatedOff())
      return;
  ...
  void STDMETHODCALLTYPE D3D11DeviceContext::RSSetViewports(
    ...
    if (unlikely(NumViewports && pViewports == nullptr))
      return;

    for (uint32_t i = 0; i < NumViewports; i++) {
      if (unlikely(!isValidD3D11Viewport(pViewports[i]))) {
        static uint32_t sInvalidViewportLogCount = 0;
        if (sInvalidViewportLogCount < 8) {
          ++sInvalidViewportLogCount;
          Logger::warn(str::format(
            "D3D11DeviceContext::RSSetViewports: Ignoring malformed viewport update at slot ",
            i,
            " (x=",
    ...
    for (uint32_t i = 0; i < NumViewports; i++) {
      ...
      const D3D11_VIEWPORT nextVp = sanitizeD3D11Viewport(pViewports[i]);

      dirty |= vp.TopLeftX != nextVp.TopLeftX
            || vp.TopLeftY != nextVp.TopLeftY
            || vp.Width    != nextVp.Width
            || vp.Height   != nextVp.Height
            || vp.MinDepth != nextVp.MinDepth
            || vp.MaxDepth != nextVp.MaxDepth;
      
      m_state.rs.viewports[i] = nextVp;
    }
  ...
  void STDMETHODCALLTYPE D3D11DeviceContext::RSSetScissorRects(
    ...
    if (unlikely(NumRects && pRects == nullptr))
      return;
  ...
  BOOL STDMETHODCALLTYPE D3D11DeviceContext::IsAnnotationEnabled() {
    return m_device->instance()->extensions().extDebugUtils && !isRenderDocAttached();
  }
  ...
  void D3D11DeviceContext::ApplyViewportState() {
    ...
    std::array<bool,       D3D11_VIEWPORT_AND_SCISSORRECT_OBJECT_COUNT_PER_PIPELINE> validViewports = { };
    ...
    for (uint32_t i = 0; i < m_state.rs.numViewports; i++) {
      ...
      if (unlikely(!isValidD3D11Viewport(vp))) {
      ...
      viewports[i] = VkViewport {
        ...
        vp.Width,   -vp.Height,
        clampViewportDepth(vp.MinDepth), clampViewportDepth(vp.MaxDepth),
      };
      validViewports[i] = true;
    }
    ...
    for (uint32_t i = 0; i < m_state.rs.numViewports; i++) {
      if (unlikely(!validViewports[i])) {
        scissors[i] = VkRect2D {
          VkOffset2D { 0, 0 },
          VkExtent2D { 0, 0 } };
      } else if (!enableScissorTest) {
        scissors[i] = VkRect2D {
          VkOffset2D { 0, 0 },
          VkExtent2D {
  ...
  void D3D11DeviceContext::CopyBuffer(
    ...
    if (pDstBuffer->Desc()->BindFlags & D3D11_BIND_INDEX_BUFFER) {
      ...
      const uint8_t* srcBytes =
        reinterpret_cast<const uint8_t*>(srcMapped.mapPtr);
      const bool shadowed =
        srcBytes != nullptr && srcMapped.length >= SrcOffset + ByteCount;
      if (shadowed) {
        pDstBuffer->UpdateIndexShadow(
      ...
      if (worthLogging && sIndexCopyLogs < 64u) {
        ...
        KENSHI_DIAGNOSTIC_INFO(str::format(
          "[D3D11Buffer][index-shadow] copy into index buffer: shadowed=",
          shadowed ? 1 : 0,
          " srcMappable=", srcBytes != nullptr ? 1 : 0,
          " srcMappedLen=", srcMapped.length,
          " srcOffset=", SrcOffset,
          " dstOffset=", DstOffset,
          " bytes=", ByteCount,
          " dstWidth=", pDstBuffer->Desc()->ByteWidth,
          " srcUsage=", uint32_t(pSrcBuffer->Desc()->Usage),
          " srcCpuFlags=", uint32_t(pSrcBuffer->Desc()->CPUAccessFlags)));
      }
    }
    ...
    if (pDstBuffer->Desc()->BindFlags & D3D11_BIND_VERTEX_BUFFER) {
      ...
      const uint8_t* srcBytes =
        reinterpret_cast<const uint8_t*>(srcMapped.mapPtr);
      if (srcBytes != nullptr && srcMapped.length >= SrcOffset + ByteCount) {
        pDstBuffer->UpdateVertexShadow(
          size_t(DstOffset), srcBytes + SrcOffset, size_t(ByteCount));
      }
    }
  ...
  void D3D11DeviceContext::UpdateBuffer(
    ...
    if ((pDstBuffer->Desc()->BindFlags & D3D11_BIND_INDEX_BUFFER)
     && Length >= 4096) {
      ...
      if (sIndexUpdateLogs < 32u) {
        ++sIndexUpdateLogs;
        KENSHI_DIAGNOSTIC_INFO(str::format(
          "[D3D11Buffer][index-shadow] UpdateBuffer into index buffer: offset=",
          Offset, " bytes=", Length,
          " dstWidth=", pDstBuffer->Desc()->ByteWidth,
          " shadowNow=", pDstBuffer->GetIndexShadowSize()));
      }
    }
  ...
  void D3D11DeviceContext::ResetState() {
    terrain_restore::demand(this, terrain_restore::Reason::Clear);
    EmitCs([] (DxvkContext* ctx) {
        ...
        if (programType == DxbcProgramType::PixelShader
         || programType == DxbcProgramType::ComputeShader) {
          ...
          for (uint32_t j = 0; j < D3D11_1_UAV_SLOT_COUNT; j++) {
            ctx->bindResourceView   (uavSlotId + j, nullptr, nullptr);
            ctx->bindResourceBuffer (ctrSlotId + j, DxvkBufferSlice());
          }
        }
      }
    });
  ...
  void D3D11DeviceContext::RestoreState() {
    terrain_profile::Scope terrainCpuRestore(terrain_profile::Stage::Restore);
    terrain_restore::RestoreTimer telemetryTimer(this);
    BindFramebuffer();

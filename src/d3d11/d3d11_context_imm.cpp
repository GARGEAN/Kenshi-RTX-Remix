...
namespace dxvk {
  ...
  void STDMETHODCALLTYPE D3D11ImmediateContext::Flush1(
          ...
          HANDLE                      hEvent) {
    if (m_parent->HasPendingInitContext())
  ...
  HRESULT STDMETHODCALLTYPE D3D11ImmediateContext::Signal(
    ...
    EmitCs([cFence = Com<D3D11Fence, false>(fence), Value] (DxvkContext* ctx) {
      ctx->flushCommandList();
      cFence->Signal(Value);
    });
  ...
  void STDMETHODCALLTYPE D3D11ImmediateContext::ExecuteCommandList(
    ...
    prepared_terrain::invalidate(prepared_terrain::Invalidation::CommandList); // Deferred writes may replay.
    D3D11DeviceLock lock = LockContext();
    terrain_restore::demand(this, terrain_restore::Reason::CommandList);
  ...
  HRESULT D3D11ImmediateContext::MapBuffer(
          ...
          UINT                        MapFlags,
          D3D11_MAPPED_SUBRESOURCE*   pMappedResource) {
    // A successful writable map exposes bytes directly; buffer Unmap is a no-op.
    // Invalidating before admission also makes failed maps conservative misses.
    if (MapType != D3D11_MAP_READ) prepared_terrain::written(pResource);
  ...
  void STDMETHODCALLTYPE D3D11ImmediateContext::SwapDeviceContextState(
    ...
    terrain_restore::demand(this, terrain_restore::Reason::StateSwap);

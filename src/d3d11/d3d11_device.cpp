...
namespace dxvk {
  ...
  void D3D11Device::PrewarmCachedGameShaders() {
    ...
    if (env::getEnvVar("DXVK_GAME_SHADER_CACHE") == "0") {
      if (prewarmLoggingEnabled())
        KENSHI_DIAGNOSTIC_INFO("[Remix-DX11][game-shader-cache] disabled by DXVK_GAME_SHADER_CACHE=0.");
      return;
    }
    ...
    try {
      ...
      for (uint32_t i = 0; i < NumElements; i++) {
        ...
        if (entry != nullptr) {
          ...
          semList.at(i).registerId  = entry->registerId;
          semList.at(i).inputSlot   = attrib.binding;
          semList.at(i).byteOffset  = attrib.offset;
          semList.at(i).componentCount = entry->componentMask.popCount();
  ...
  HRESULT STDMETHODCALLTYPE D3D11Device::CreateFence(
    ...
    if (Flags != D3D11_FENCE_FLAG_NONE) {
      static bool s_warnShown = false;
      if (!std::exchange(s_warnShown, true))
        Logger::warn("D3D11Device::CreateFence: Creating local compatibility fence for non-zero flags");
    }

    D3D11Fence* fence = new D3D11Fence(this, InitialValue);
    HRESULT hr = fence->QueryInterface(ReturnedInterface, ppFence);
  ...
  bool D3D11Device::HasPendingInitContext() const {
    return m_initializer->HasPendingWork();
  }
  ...
  HRESULT D3D11Device::CreateShaderModule(
    ...
    if (pClassLinkage != nullptr) {
      ...
      std::call_once(warning, [] {
        Logger::warn("D3D11Device::CreateShaderModule: Class linkage not supported (further occurrences suppressed)");
      });
    }
